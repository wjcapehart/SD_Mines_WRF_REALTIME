...
;
 ...
 time_start_in_sec = systime(/seconds)
   ...
   WRF_OUTSTORE = '/cyclone1/WRF_REALTIME/OUTPUT/WRF39_KUNR_3DOM_WSM6_NOAH_MYNN2_KF2_DUDHIA_NAM218-09km_150x150/'
   ...
   GRIB_NCAR_DIR         = '/squall1/gempak-data/grib/nam'
   GRIB_PREFIX           = 'nam212_'
   ...
   WPS_LNKGRID_CMD       =  "source ~wjc/.bash_profile &&   "+ wrf_program_root_dir+'WPS/link_grib.csh '
   WPS_UNGRIB_CMD        =  "source ~wjc/.bash_profile &&   "+ wrf_program_root_dir+'WPS/ungrib.exe ' ;> ungrib.output'
   WPS_METGRID_CMD       =  "source ~wjc/.bash_profile &&   "+ wrf_program_root_dir+'WPS/metgrid.exe ';> metgrid.output'
   ...
   WRF_REAL_CMD          = 'source ~wjc/.bash_profile &&  mpiexec --machinefile /home/wjc/nodeswrf --display-map --display-allocation --n '+ NUMBEROFPROCS + ' ./real.exe    '
   WRF_WRF_CMD           = 'source ~wjc/.bash_profile &&  mpiexec --machinefile /home/wjc/nodeswrf --display-map --display-allocation --n '+ NUMBEROFPROCS + ' ./wrf.exe    '
...
;
         ...
         website = "ftp://ftpprd.ncep.noaa.gov/pub/data/nccf/com/nam/prod/nam." + ncdc_dir1
         ...
         FOR TT = 0L, NT_UNGRIB-1L DO $
             GRIB_FILE_LOCAL(TT) =  pref+string(HOUR_UNGRIB(0),fx(TT),format="('.t',I2.2,'z.conusnest.hiresf',I2.2,'.tm00.grib2')")


         FOR TT = 0L, NT_UNGRIB-1L DO $
              GRIB_FILE_MSS(TT) = website+ "/" + GRIB_FILE_LOCAL(TT)




         FOR TT = 0L, NT_UNGRIB-1L DO $
            print, GRIB_FILE_MSS(TT)

         FOR TT = 0L, NT_UNGRIB-1L DO $
            print, GRIB_FILE_LOCAL(TT)

            FOR TT = 0L, NT_UNGRIB-1L DO BEGIN
               print, "axel --quiet " + GRIB_FILE_MSS(TT)
               spawn, "axel --quiet " + GRIB_FILE_MSS(TT)
            ENDFOR
   ...
   SPAWN, 'mkdir -v '   + WRF_OUTSTORE
          ...
          WRF_SM_FILENAMES = "wrfout_d01_" +  SUBOUT_DATE_STRING_SM + ".nc"
          FOR TT = 0L, NT_SUBOUTFILES-1L DO $
              SPAWN, 'source ~wjc/.bash_profile && /usr/local/netcdf/bin/nccopy -4 -d 5 ' + WRF_FILENAMES(TT) + ' ' + WRF_SM_FILENAMES(TT)
          FOR TT = 0L, NT_SUBOUTFILES-1L DO $
          ...
          SPAWN, "source ~wjc/.bash_profile && ncl "+wrf_home_dir+" ts2nc_autoread_d01.ncl"
          SPAWN, "source ~wjc/.bash_profile && ncl "+wrf_home_dir+" ts2nc_autoread_d02.ncl"
   ...
   spawn, 'cp -v /home/wjc/WRF_REALTIME/background_error_file.txt  /projects/WRF_REALTIME/OUTPUT_PNG/currentRAP/'
   SPAWN, "scp  /home/wjc/WRF_REALTIME/background_error_file.txt wjc@kyrill:/var/www/html/firemet/wrf_rap"
...
final_clocktime = systime(/UTC) + " UTC"
   ...
   ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
   ;; run UPP
      spawn, "source ~wjc/.bash_profile &&  " + wrf_program_root_dir + "/UPPV3.2/scripts/run_unipost_SDSMT >& /home/wjc/WRF_REALTIME/UPP.log"
